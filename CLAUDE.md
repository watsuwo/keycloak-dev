...
> 初回アクセス時、ブラウザに自己署名証明書の警告が出ます。これは正常です。「詳細設定」→「アクセスする」で進めるか、`certs/local-cert.pem` をOS/ブラウザの信頼ストアに追加すれば警告を消せます。
...
│   ├── traefik.yml
│   └── dynamic/tls.yml
├── certs/                       # 自己署名証明書 (gitignore対象 / make certs で生成)
├── scripts/                     # 運用ヘルパー
│   └── generate-certs.sh
├── providers/                   # SPI実装 (Java) — Phase 2で本格整備
├── themes/                      # FreeMarker/CSS/JS — Phase 2で本格整備
└── realms/                      # Realm JSON (起動時にimport) — Phase 2で本格整備
```
...
`.env` / `compose.override.yaml` / `certs/` はgit管理外。
...
- **証明書エラーで進めない** → 自己署名なので警告は正常。回避するか、`certs/local-cert.pem` を信頼ストアに追加。
